    ...
    <>
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
          ...
          <span style={{ fontWeight: '300', fontSize: '1.2rem', letterSpacing: '1px' }}>Prototipica</span>
        </div>
        <button onClick={() => setAbierto(!abierto)} style={{ background: 'none', border: 'none', fontSize: '1.6rem', cursor: 'pointer', padding: '0.4rem' }}>
          <FaBars />
        </button>
      </header>
      ...
      {abierto && (
        <div style={{
          ...
          <nav style={{ width: '100%', maxWidth: '400px' }}>
            {menuItems.map(item => (
              <button key={item.id} onClick={() => navegar(item.id)} style={{
                ...
                padding: '1rem 1.5rem', fontSize: '1.4rem',
          ...
          <p style={{ position: 'absolute', bottom: '2rem', color: '#999', fontSize: '0.85rem' }}>pdabasel1@gmail.com</p>
        </div>
      )}
    </>
  );
